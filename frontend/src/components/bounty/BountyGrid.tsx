      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="relative mb-6">
          ...
          <input
            ...
            placeholder="Search bounties by title, description, language, or repo..."
            className="w-full bg-forge-800 border border-border rounded-lg pl-10 pr-10 py-2.5 text-sm text-text-primary placeholder:text-text-muted focus:border-emerald focus:outline-none transition-colors duration-150"
          />
          {searchQuery && (
            <button
        ...
        {!isLoading && !isError && filteredBounties.length === 0 && (
          <div className="text-center py-16">
            ...
            <p className="text-text-muted text-sm">
              {debouncedQuery ? `No results for "${debouncedQuery}". Try a different search term.` : activeSkill !== 'All' ? `Try a different language filter.` : 'Check back soon for new bounties.'}
            </p>
          </div>
        )}
