...
export function BountyGrid() {
  ...
  const debouncedSetQuery = React.useMemo(() => debounce((q: string) => setDebouncedQuery(q), 300), []);
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="relative mb-6">
          ...
          <input
            ...
            className="w-full bg-forge-800 border border-border rounded-lg pl-10 pr-10 py-2.5 text-sm text-text-primary placeholder-text-muted focus:border-emerald focus:outline-none transition-colors duration-150"
          />
          {searchQuery && (
            <button
              onClick={clearSearch}
        ...
        {!isLoading && !isError && allBounties.length > 0 && filteredBounties.length === 0 && (
          <div className="text-center py-16">
            <p className="text-text-muted text-lg mb-2">No results for "{debouncedQuery}"</p>
            <p className="text-text-muted text-sm">Try a different search term or clear the filter.</p>
          </div>
        )}
