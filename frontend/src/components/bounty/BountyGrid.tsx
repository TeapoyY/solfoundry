...
function filterBounties(bounties: Bounty[], query: string): Bounty[] {
...
export function BountyGrid() {
  ...
  const filteredBounties = useMemo(
    () => filterBounties(allBounties, debouncedSearch),
    ...
    <section id="bounties" className="py-16 md:py-24">
      <div className="max-w-7xl mx-auto px-4">
        ...
        {!isLoading && !isError && allBounties.length > 0 && filteredBounties.length === 0 && (
          <div className="text-center py-16">
            <p className="text-text-muted text-lg mb-2">No bounties match &ldquo;{searchQuery}&rdquo;</p>
            <button
              onClick={() => setSearchQuery('')}
              className="text-sm text-emerald hover:text-emerald-light transition-colors duration-150"
            >
              Clear search
            </button>
          </div>
