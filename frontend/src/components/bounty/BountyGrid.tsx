      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="relative mb-6">
          ...
          <input
            ...
            className="w-full pl-10 pr-10 py-2.5 rounded-lg bg-forge-800 border border-border text-sm text-text-primary placeholder-text-muted focus:border-emerald outline-none transition-colors duration-150"
          />
          {searchInput && (
            <button
        ...
        {!isLoading && !isError && allBounties.length > 0 && filteredBounties.length === 0 && (
          <div className="text-center py-16">
            <p className="text-text-muted text-lg mb-2">No bounties match "{debouncedSearch}"</p>
            <button
              onClick={() => setSearchInput('')}
              className="text-emerald text-sm font-medium hover:text-emerald/80 transition-colors"
            >
              Clear search
            </button>
          </div>
