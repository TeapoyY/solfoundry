 ...
 * The state is stored in sessionStorage and validated on callback to prevent CSRF attacks.
 *
 ...
 * @param state - CSRF state token for security (auto-generated if not provided)
 */
 ...
 * Falls back to building the URL directly if the backend is unavailable (network error or 5xx/503/504/404).
 * Only falls back for genuine network errors — throws on malformed responses.
 *
 ...
 * Clears local tokens afterwards using a finally block to ensure cleanup
 * happens regardless of whether the server call succeeds or fails.
 */
